...
<body>
  <br><br>
  <button type="button" class="button" style="width:300px; background:#46BEFF; padding:10px; margin-left : 200px ; display: inline-block;"onclick="location.href='managingCar_select.jsp'">Campcar Main</button>
  <button type="button" class="button" style="width:300px; background:#915921; padding:10px; margin-left : 600px ;color:white; display: inline-block;"onclick="location.href='adminMain.jsp'">Manager Main</button>
  <br><br>
<%  
...
ResultSet myRS = null;
  try{
    ...
    String sql = "select c.cp_name, c.cp_type, c.cp_num, TO_CHAR(c.cp_date, 'YYYY-MM-DD') cp_date2, c.cp_count, c.cp_info, LTRIM(TO_CHAR(c.cp_cost,'999,999,999')) cp_cost2, c.cp_cost cp_cost3,  y.c_name from campcar c, company y WHERE c.c_id = y.c_id AND c.CP_STATE='미사용' order by c.c_id";
            ...
            <div class="card">
              ...
              <div class="rightside">
                <form action="">
                  ...
                  <p></p>
                  <button type="button" class="button" onclick="location.href='managingCar_updateForm.jsp?name=<%=cpName%>&type=<%=cpType%>&num=<%=cpNum%>&date=<%=cpDate%>&count=<%=cpCount%>&info=<%=cpInfo%>&cost=<%=cpCost%>'">update</button>
                  <button type="button" class="button2" onclick="location.href='managingCar_usePro.jsp?num=<%=cpNum%>'">use</button>
                </form>
              </div>
            </div>
          </div>
