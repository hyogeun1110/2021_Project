...
String carSelect = request.getParameter("carSelect");

String[] carValues = carSelect.split("/");
String originalCar = carValues[0];  //여기다가 원래 고객의 차량변호 전역변수
String originalName = carValues[1]; //여기다가 원래 고객의 차량이름 전역변수
String originalCount = carValues[2]; //여기다가 원래 고객의 인승 전역변수
String originalCost = carValues[3]; //여기다가 원래 고객의 비용 전역변수
...
<body style="background-image:url('img/6.jpg'); background-size:cover;">
  ...
  <div class="container">
    <form method="post" action="ReservationChangePro.jsp">
      <div class="middle">
...
Connection conn = null;
 ...
 try{
  ...
  String sql2 = "SELECT * FROM CAMPCAR order by c_id";
      ...
      %>
            <input type="hidden" name="carSelect2" value="<%=originalCar%>">
            <select class="form-select1 form-select-lg mb-3" aria-label=".form-select-lg example" id="carSelect" name="carSelect1" style="color:gray; font-size: 16px;" disabled>
      ...
      %>
                ...
                <table class="table"  style="position: relative; left: 00px; top:50px;  ">
                    <thead>
                        <tr>
...
<%
  sql = "select b.cpu_id, b.cp_num, c.cp_name, c.cp_count, c.cp_cost, b.cs_license, b.cpu_usestart, b.cpu_useend, b.cpu_useday, b.cpu_cost from CAMPCAR_USE b,CAMPCAR c where b.CP_NUM = c.CP_NUM and b.CPU_ID = ?";
      pstmt = conn.prepareStatement(sql); 
      pstmt.setString(1,k);
      myRS = pstmt.executeQuery();
      if(myRS != null){
  ...
  <div hidden>
  <span class="input-group-text" id="inputGroup-sizing-sm" >날짜</span>
  <input type="text" class="form-control" aria-label="Sizing example input"  id = "d1" name = "d1" aria-describedby="inputGroup-sizing-sm" hidden>
</div>

 <div hidden>
  <span class="input-group-text" id="inputGroup-sizing-sm">총가격</span>
  <input type="text" class="form-control" aria-label="Sizing example input"  id = "d2" name = "d2" aria-describedby="inputGroup-sizing-sm" hidden>
</div>
...
<input type="button" class="btnok btn-outline-primary" onclick="location='Main.jsp'" style="top:248px; left:-100px;" value="처음으로">
<input type="button" class="btnok btn-outline-primary"  style="top:248px; left:100px;" onclick="location='ReservationCancel.jsp?k=<%=CPU_ID%>'" value="예약취소">
<input type="submit" class="btnok btn-primary" style="left:105px;" value="변경하기">
